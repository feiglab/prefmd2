...
"""
Modified version of the IterationMaster.py script from the Rosetta package
(found in the $ROSETTA3/main/source/scripts/python/public/iterative_hybridize
directory). It is adapted to run in the PREFMD pipeline.
"""
...
#ENVIRONMENT SETUP
ROSETTABIN = '%s/main/source/bin'%os.environ['ROSETTA_HOME']
DB = '%s/main/database'%os.environ['ROSETTA_HOME']
...
class Iterator:
    ...
    def get_nodes(self):
        ...
        if self.opt.nodefile != '':
            nodes = []
            with open(self.opt.nodefile) as fp:
                for line in fp:
                    x = line.strip()
                    if len(x) != '':
                        nodes.append(x)
            #
            use_curr_node_only = False
            if len(nodes) == 1:
                single_node = nodes[0].split("/")[1]
                if single_node == 'localhost' or single_node == self.host:
                    use_curr_node_only = True
            #
            if use_curr_node_only:
                self.PARALLEL = '%s -j %s'%(PARALLELBIN, nodes[0].split("/")[0])
            else:
                self.PARALLEL = '%s --sshdelay 0.01 -S %s'%(PARALLELBIN, ','.join(nodes))
        else:
            sys.exit(ERRORMSG)
...
######## Preparation
    ...
    def Prepare_iter(self):
        ...
        n_init = len(glob.glob('iter%d.*pdb'%self.it))
        if n_init == 0:
            sys.exit('ERROR: pdbs not extracted correctly!')
        self.opt.nseed = min(self.opt.nseed, n_init)
...
######## Structure Generation
    ...
    def make_joblist(self):
        ...
        cmds = []
        form = '%s/runhyb.sh %s %s %s %s %s %d %s %s %s %s'
        for icomb,comb in enumerate(self.combs):
            ...
            cmd = form%(SCRIPTDIR,self.pwd,ROSETTABIN,DB,EXTENSION,SCRIPTDIR,
        ...
        while True:
            ...
            if os.path.exists(GENSILENT):
                ...
                if self.opt.debug:
                    ...
                    os.system('echo "%s --workdir . :::: alljobs" > run.cmd'%self.PARALLEL)
                    os.system("%s --workdir . :::: alljobs &> run.log"%self.PARALLEL)
